...
async function bootstrap(): Promise<void> {
  ...
  const limit = resolveLimit(process.argv.slice(2));
  ...
  try {
    logger.log(`resolved limit=${limit}`, context);
    ...
    logger.log(
      `summary: pending=${summary.pendingFound}, cacheSuccessHit=${summary.cacheSuccessHit}, cacheFailureHit=${summary.cacheFailureHit}, apiSuccess=${summary.apiSuccess}, apiFailure=${summary.apiFailure}, skipped=${summary.skipped}`,
      context,
    );
  } finally {
    client.release();
